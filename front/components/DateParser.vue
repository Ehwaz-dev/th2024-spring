...
const myinput = ref('');
const parsedDate = ref({ title: '', date: '' });
const examples = ref([
...
const jsParseDate = (title) => {
  ...
  if (matches) {
    ...
    let matches3 = title.match(shablon); //найти месяц
    shablon = /(\d{1,2})/g;
    let matches4 = matches[0].match(shablon); //найти дату
...
watch(myinput, (newVal) => {
  parsedDate.value = jsParseDate(newVal);
});
</script>
...
<template>
  <div>
    ...
    <span>Распознано как:</span> <b>{{ parsedDate.title }}</b><br/>
    <span>Дата:</span> <b>{{ parsedDate.date }}</b><br/>
    <hr>
    Кликните для проверки примеров:
    <ol>
      <li v-for="example in examples" :key="example.title" @click="myinput = example.title">{{ example.title }}</li>
    </ol>
  </div>
</template>
